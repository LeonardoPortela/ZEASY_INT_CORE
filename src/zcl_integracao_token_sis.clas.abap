class ZCL_INTEGRACAO_TOKEN_SIS definition
  public
  final
  create public .
...
public section.
  ...
  interfaces ZIF_INTEGRACAO_TOKEN_SIS .
...
CLASS ZCL_INTEGRACAO_TOKEN_SIS IMPLEMENTATION.
  ...
  METHOD zif_integracao_token_sis~get_instance.

    IF zif_integracao_token_sis~at_if_integracao_token IS NOT BOUND.
      CREATE OBJECT zif_integracao_token_sis~at_if_integracao_token
        TYPE zcl_integracao_token_sis.
    ENDIF.

    r_if_integracao_token = zif_integracao_token_sis~at_if_integracao_token.

  ENDMETHOD.


  METHOD zif_integracao_token_sis~get_token.

*// Tipos de Dados
    TYPES: BEGIN OF ty_token,
             grant__type TYPE string,
    ...
    IF sy-subrc IS INITIAL.
      ...
      IF sy-subrc IS INITIAL.
        ...
        VALUE #(
                  ...
                  ( name  = 'Content-Type'    value = ls_webservice-content_type )
               ).
...
BREAK-POINT.
    ...
    me->zif_integracao_token_sis~set_ds_url( ).
    me->zif_integracao_token_sis~set_send_msg( IMPORTING
  ...
  METHOD zif_integracao_token_sis~set_ds_url.
    ...
    READ TABLE lt_werbservice INTO wa_webservice WITH KEY service = 'SIS_TOKEN' BINARY SEARCH.
    IF sy-subrc IS INITIAL.
      me->zif_integracao_inject~at_info_request_http-ds_url = |{ wa_webservice-url }{ wa_webservice-add01 }={ wa_webservice-add02 }|.
    ENDIF.
  ...
  METHOD zif_integracao_token_sis~set_send_msg.
